...
func Test_Hour1(t *testing.T) {
	field := Hour(1)
	got := field.MustFormat()
	assert.Equal(t, "1", got)
}

func Test_Hour2(t *testing.T) {
	field := Hour(-1)
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "invalid hour (-1) must be between 0 and 23")
}

func Test_Hour3(t *testing.T) {
	field := Hour(24)
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "invalid hour (24) must be between 0 and 23")
}
...
func Test_List6(t *testing.T) {
	...
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "no hour values were passed")
}
...
func Test_Range5(t *testing.T) {
	...
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "invalid hour range, 'from' (4) must be less than 'to' (2)")
}
...
func Test_EveryX3(t *testing.T) {
	...
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "invalid hour (0) must be between 1 and 12")
}
...
func Test_EveryX4(t *testing.T) {
	...
	got, err := field.Format()
	assert.Equal(t, "", got)
	assert.ErrorContains(t, err, "invalid hour (13) must be between 1 and 12")
}
